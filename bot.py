...
user_anketa = {}
admin_state = {}
ariza_pending = {}  # Rahbarning tasdiqlashini kutayotgan arizalar
...
# Ariza sarlavhasi (HR / rahbarga boradigan xabar uchun)
ARIZA_HEADER = {
    "uz": "YANGI ARIZA!",
    "ru": "НОВАЯ ЗАЯВКА!",
    "en": "NEW APPLICATION!",
...
FIELD_LABELS = {
    "uz": [
        ...
        ("Hozirgi yashash manzili", "yashash_joy"),
        ("Ish tajribasi", "ish_tajriba"),
        ("Asosiy ish vazifalari nimalardan iborat bo'lgan", "ish_vazifa"),
    ...
    "ru": [
        ...
        ("Место рождения", "tug_joy"),
        ("Текущий адрес проживания", "yashash_joy"),
        ("Опыт работы", "ish_tajriba"),
        ("В чём заключались основные должностные обязанности", "ish_vazifa"),
        ("Поездки за границу", "chet_safari"),
        ("Семейное положение", "oilaviy_holat"),
        ("Судимость", "sudlanganmi"),
        ("Автомобиль", "avtomobil"),
        ("Водительские права", "haydovchilik"),
        ("Узбекский язык", "uzbek_tili"),
        ("Русский язык", "rus_tili"),
        ("Английский язык", "ingliz_tili"),
        ("Интересы", "qiziqish"),
        ("Компьютер", "kompyuter"),
        ("Рекомендация", "tavsiya"),
        ("Предыдущая зарплата", "oldingi_maosh"),
        ("Ожидаемая зарплата", "kutilayotgan_maosh"),
        ("Срок", "muddat"),
        ("Смена", "smena"),
    ],
    "en": [
        ...
        ("Age", "yosh"),
        ("Nationality", "millat"),
        ("Place of birth", "tug_joy"),
        ("Current residential address", "yashash_joy"),
        ("Work experience", "ish_tajriba"),
        ("What were your main job responsibilities", "ish_vazifa"),
        ("Travel abroad", "chet_safari"),
        ("Marital status", "oilaviy_holat"),
        ("Criminal record", "sudlanganmi"),
        ("Car", "avtomobil"),
        ("Driver's license", "haydovchilik"),
        ("Uzbek language", "uzbek_tili"),
        ("Russian language", "rus_tili"),
        ("English language", "ingliz_tili"),
        ("Interests", "qiziqish"),
        ("Computer", "kompyuter"),
        ("Reference", "tavsiya"),
        ("Previous salary", "oldingi_maosh"),
        ("Expected salary", "kutilayotgan_maosh"),
        ("Duration", "muddat"),
        ("Shift", "smena"),
    ],
}
...
async def anketa_callback(update, context):
    ...
    if query.data == "anketa_confirm":
        ...
        try:
            # Ariza ID yaratish (timestamp + user_id)
            ariza_id = f"{user_id}_{int(__import__('time').time())}"
            ariza_pending[ariza_id] = {
                "data": data,
                "user_id": user_id,
                "lang": lang,
                "msg": msg,
            ...
            # RAHBARNI yuborish tasdiqlash tugmalari bilan
            approve_keyboard = InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ Qabul qilish", callback_data=f"ariza_approve_{ariza_id}"),
                InlineKeyboardButton("❌ Rad etish", callback_data=f"ariza_reject_{ariza_id}")
            ]])

            await context.bot.send_message(
                chat_id=ADMIN_RAHBAR,
                text=f"🔔 YANGI ARIZA (RAHBARNING TASDIQLASHINI KUTMOQDA)\n\n{msg}",
                reply_markup=approve_keyboard
            )
            logger.info(f"Ariza rahbarga yuborildi: {ariza_id}")
            ...
            logger.error(f"Rahbarga xato: {e}")
...
async def ariza_approval_callback(update, context):
    """Rahbar arizani qabul qilish/rad etish"""
    query = update.callback_query
    await query.answer()

    if query.data.startswith("ariza_approve_"):
        ariza_id = query.data.replace("ariza_approve_", "")

        if ariza_id not in ariza_pending:
            await query.edit_message_text("❌ Ariza topilmadi yoki o'z vaqtida bo'ldi")
            return

        app_data = ariza_pending[ariza_id]
        data = app_data["data"]
        lang = app_data["lang"]
        msg = app_data["msg"]
        user_id = app_data["user_id"]
        rasm_id = app_data["rasm_id"]

        try:
            # HR ga yuborish
            await context.bot.send_message(
                chat_id=ADMIN_HR,
                text=f"✅ RAHBAR TASDIQLADI\n\n{msg}"
            )
            if rasm_id:
                await context.bot.send_photo(
                    chat_id=ADMIN_HR,
            ...
            # Database ga saqlash
            db_query("INSERT INTO arizalar (ism, telefon, lavozim, smena, sana, holat) VALUES (?,?,?,?,?,?)",
                     (data.get('ism'), data.get('telefon', '—'), "—",
                      data.get('smena'), datetime.now().strftime("%d.%m.%Y"), "qabul"))

            # Rahbarning xabari o'zgartiriladi
            await query.edit_message_text(f"✅ TASDIQLANDI VA HR GA YUBORILDI\n\n{msg}")

            logger.info(f"Ariza qabul qilindi: {ariza_id}")

        except Exception as e:
            logger.error(f"HR ga yuborish xatosi: {e}")
            await query.edit_message_text("❌ Xatolik yuz berdi")

        ariza_pending.pop(ariza_id, None)

    elif query.data.startswith("ariza_reject_"):
        ariza_id = query.data.replace("ariza_reject_", "")
        ...
        try:
            # Applicantga xabar
            if lang == "uz":
                reject_msg = "❌ Kechirasiz, arizangiz rad etildi.\n\nBoshqa omad tilaymiz!"
            elif lang == "ru":
                reject_msg = "❌ К сожалению, ваша анкета отклонена.\n\nУдачи в других местах!"
            else:
                reject_msg = "❌ Sorry, your application was rejected.\n\nGood luck elsewhere!"
            ...
            db_query("INSERT INTO arizalar (ism, telefon, lavozim, smena, sana, holat) VALUES (?,?,?,?,?,?)",
                     (data.get('ism'), data.get('telefon', '—'), "—",
                      data.get('smena'), datetime.now().strftime("%d.%m.%Y"), "rad"))

            # Rahbarning xabari o'zgartiriladi
            await query.edit_message_text(f"❌ RAD ETILDI\n\n{msg}")

            logger.info(f"Ariza rad etildi: {ariza_id}")

        except Exception as e:
            logger.error(f"Rad xatosi: {e}")
            await query.edit_message_text("❌ Xatolik yuz berdi")
...
# Suhbatni tozalash tugmalari (barcha tillarda)
CLEAR_BUTTONS = {"🗑️ Suhbatni tozalash", "🗑️ Очистить чат", "🗑️ Clear Chat"}
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_text in CLEAR_BUTTONS:
        ...
        await update.message.reply_text(get_text(user_id, "tozalandi"), reply_markup=get_menu(user_id))
        return
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(ariza_approval_callback, pattern="^ariza_"))
